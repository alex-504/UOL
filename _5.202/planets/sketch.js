// // arrays 5.2 (Basic array example)

// var sizes = [4,9,10,5,110,95,40,40];
// var names = ["Mercury", "Venus", "Earth", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune"];
// var colors = ["grey", "brown", "green", "red", "sienna", "pink", "blue", "cyan"];

// function setup()
// {
// 	createCanvas(1000, 600);
// }

// function draw()
// {
// 	background(0);
// 	textAlign(CENTER);

		
// 	fill(colors[0]);
// 	ellipse(50, height/2, sizes[0]);
// 	fill(255);
// 	text(names[0], 50, height/2 + 150);
	
// 	fill(colors[1]);
// 	ellipse(175, height/2, sizes[1])
// 	fill(255);
// 	text(names[1], 175, height/2 + 150);
	
// 	fill(colors[2]);
// 	ellipse(300, height/2, sizes[2])
// 	fill(255);
// 	text(names[2], 300, height/2 + 150);
	
// 	fill(colors[3]);
// 	ellipse(425, height/2, sizes[3])
// 	fill(255);
// 	text(names[3], 425, height/2 + 150);
	
// 	fill(colors[4]);
// 	ellipse(550, height/2, sizes[4])
// 	fill(255);
// 	text(names[4], 550, height/2 + 150);
	
// 	fill(colors[5]);
// 	ellipse(675, height/2, sizes[5])
// 	fill(255);
// 	text(names[5], 675, height/2 + 150);
	
// 	fill(colors[6]);
// 	ellipse(800, height/2, sizes[6])
// 	fill(255);
// 	text(names[6], 800, height/2 + 150);
	
// 	fill(colors[7]);
// 	ellipse(925, height/2, sizes[7])
// 	fill(255);
// 	text(names[7], 925, height/2 + 150);

// }


// arrays 5.2 (Arrays with "for" loopin)

// var months = [ "January", "February",  "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ];
// var days = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

// for(var i = 0; i < 12; i++){
//   console.log(months[i] + " has " + days[i] + " days.");
// }

// var a = [2, 0, 1];
// var b = [20, 30, 40]

// if( b[a[0]] == 40){
//   console.log("Access granted!");
// }

// var lyrics = ["You", "say", "yes", "I", "no", "stop", "go"];

// for(var i = 0; i < 8; i++){
//   console.log(lyrics.i);
// }


// Errors with Loops
var myArray = [1,2,3,4,5,6,7];

function setup()
{
	console.log(myArray);
	for (let i = 0; i < 40; i++) // missing i++ will trigger an infinite loop (and crash the browser)
	{
		console.log(i)
		
	}
}